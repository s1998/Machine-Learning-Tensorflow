...
class BrnnForPsspModelOne:
  def __init__(self,
    ...
    self.loss_reduced = ( tf.reduce_sum(self.loss_masked) 
                        / self.no_of_entries_unmasked )
	
    self.get_equal_unmasked = tf.equal(
		tf.argmax(
			self.input_y_o_r, 
			1), 
		tf.argmax(
    ...
    self.get_equal = tf.multiply(
		tf.cast(self.get_equal_unmasked, tf.float64),
		self.input_msks_r)
    self.accuracy = ( tf.reduce_sum(tf.cast(self.get_equal, tf.float64))
                    / self.no_of_entries_unmasked)
