...
class RnnForPfcModelFive:
	def __init__(self, 
		...
		self.freq = tf.placeholder(tf.float32, [None])
		...
		self.loss_unwieghted = (
					tf.nn.softmax_cross_entropy_with_logits(logits=self.y_predicted, labels=self.y_input_o))
		self.loss_wieghted = tf.multiply(self.loss_unwieghted, self.freq_inv)
		self.loss_reduced = tf.reduce_mean(self.loss_wieghted)
...
def train_on_train_data(epoch, model, data_train, data_test):
	...
	for batch_no in [167, 160, 120, 80, 40, 10]:
		...
		if use_optimizer == 1:
			model.optimize_1(x_padded, y, seq_length, freq)
		else:
			model.optimize_2(x_padded, y, seq_length, freq)
		accuracy = model.cross_validate(x_padded, y, seq_length, freq)
		print("Training data accuracy : ", accuracy)
		print("Training data loss     : ", model.get_loss(x_padded, y, seq_length, freq))
...
if __name__=="__main__":
	...
	model = RnnForPfcModelFive(num_classes = 498, hidden_units=100, learning_rate=0.001)
